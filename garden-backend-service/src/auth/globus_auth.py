...
@cached(cache=TTLCache(maxsize=100, ttl=5 * 60))
def introspect_mdf_token(token: str, log: bool = True) -> globus_sdk.GlobusHTTPResponse:
    """Introspect a token with MDF auth client and return the response data.
    Needed to check if auth is using MDF client creds.
    """
    client = get_mdf_auth_client()
    auth_data = client.oauth2_token_introspect(
        token, include="identity_set,identity_set_detail"
    )
...
@lru_cache
def get_mdf_auth_client() -> globus_sdk.ConfidentialAppAuthClient:
    """Create an AuthClient for the service."""
    settings = get_settings()
    return globus_sdk.ConfidentialAppAuthClient(
        settings.MDF_API_CLIENT_ID, settings.MDF_API_CLIENT_SECRET
    )
