...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_redeploy_modal_app_not_super_user(
    client,
    mock_db_session,
    mock_auth_state,  # Regular user, not super user
    mock_modal_app_create_request_one_function,
    override_sandboxed_functions,
):
    # First create a modal app
    post_response = await post_modal_app(
        client, mock_modal_app_create_request_one_function
    )
    app_id = post_response["id"]

    # Try to redeploy as non-super user
    response = await client.post(f"/modal-apps/redeploy/{app_id}")
    assert response.status_code == 403
    assert (
        "You must be a super user to perform this action" in response.json()["detail"]
    )


@pytest.mark.asyncio
@pytest.mark.integration
async def test_redeploy_modal_app_not_found(
    client,
    mock_db_session,
    mock_modal_publisher_auth_state,
    override_sandboxed_functions,
):
    # Try to redeploy non-existent app
    response = await client.post("/modal-apps/redeploy/999999")
    assert response.status_code == 404
    assert "Modal App not found with id 999999" in response.json()["detail"]
