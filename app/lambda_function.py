...
@app.route("/garden-search-record", methods=["POST"])
def publish(event, context, kwargs):
    ...
    while task_result["state"] not in {"FAILED", "SUCCESS"}:
        if not max_intervals:
            return {
                "statusCode": 408,  # there isn't a perfect analog here, so perhaps a simple 500 may even be better
                "body": json.dumps(
                    {
        ...
        sleep(2)  # suitable wait time and timeout duration?
        max_intervals -= 1
        task_result = search_client.get_task(publish_result["task_id"])
